...
void loop() {
  ...
  // Prints PWM values from the radio input (expected range: 1000 to 2000)
  printRadioData(channel_1_pwm, channel_2_pwm, channel_3_pwm, channel_4_pwm, channel_5_pwm, channel_6_pwm);

  // Prints the desired state of the vehicle (expected +/- maxAXIS for roll, pitch, yaw; 0 to 1 for throttle)
  // printDesiredState(thro_des, roll_des, pitch_des, yaw_des);

  // Prints filtered gyro data from the IMU (expected: ~ -250 to 250, 0 at rest)
  // printGyroData(GyroX, GyroY, GyroZ);

  // Prints filtered accelerometer data from the IMU (expected: ~ -2 to 2; x, y 0 when level, z 1 when level)
  // printAccelData(AccX, AccY, AccZ);

  // Prints filtered magnetometer data from the IMU (expected ~ -300 to 300)
  // printMagData(MagX, MagY, MagZ);

  // Prints roll, pitch, and yaw angles from Madgwick filter (expected in degrees)
  // printRollPitchYaw(roll_IMU, pitch_IMU, yaw_IMU);

  // Prints the PID controller outputs for stabilization and setpoints
  // printPIDoutput(roll_PID, pitch_PID, yaw_PID);

  // Prints the motor command values being sent to motor controllers (expected range 120 to 250)
  // printMotorCommands(m1_command_PWM, m2_command_PWM, m3_command_PWM, m4_command_PWM, m5_command_PWM, m6_command_PWM);

  // Prints the servo command values being sent to servos (expected range 0 to 180)
  // printServoCommands(s1_command_PWM, s2_command_PWM, s3_command_PWM, s4_command_PWM, s5_command_PWM, s6_command_PWM, s7_command_PWM);

  // Prints the loop rate (time between iterations in microseconds)
  // printLoopRate(dt);
  // Get arming status
